...
Using the supercritical two-dimensional flow past a circular cylinder at Reynolds number of 100 as an example, the top row in figure @fig:timings depicts the streamwise (*u*) velocity distribution of the converged flow field (top) as well as the pointwise squared difference between the the solutions obtained with the two solution techniques with a tolerance of $\varepsilon = 10^{-8}$, namely `LightKrylov`'s Newton-GMRES solver and the selective frequency damping algorithm [@pof-sfd] (often considered as the gold standard in this community). Note that the error norm is different for each method. The Newton-GMRES iteration uses the residual of the nonlinear integration over a time horizon of $\tau = 1.0$ integrated over the domain as a convergence measure, which is more strict than the point-wise measure used in the implementation of SFD.

Using the spectral element solver `Nek5000`, the governing equations are discretized with 1996 elements using a 5-th order polynomial approximation in each direction resulting in 71 856 grid points and roughly 175 000 degrees of freedom (i.e. the two velocity components as well as the pressure). 
Both solvers make use of the same initial condition and computations were run in parallel on 12 Intel Core Ultra 7 processors.
Note that the Newton-GMRES solver makes use of the *time-stepper* approach recently reviewed in @frantz-2023 while the selective frequency damping solver uses the implementation provided by the [`KTH Framework`](https://github.com/KTH-Nek5000/KTH_Framework) toolbox [@kth-framework]. Leveraging the abstract interfaces provided by `LightKrylov`, the implementation of the Newton-GMRES solver in `neklab` required no modifications of the `Nek5000` solver. A summary of the computational cost is shown in table 1 of [Figure @fig:timings]. The code to run this example can found online [here](???).

## Computing the leading eigenpair of the linearized Navier-Stokes operator

Investigating the spectral properties of the linearized Navier-Stokes operator is critical to better understand the transition to unsteadiness and turbulence in many flow applications. Using once again the two-dimensional cylinder flow as an example, the table below shows the wall-clock time needed to converge the first pair of complex-conjugate eigenvalues. The two solvers considered include once again `neklab` and `KTH Framework`. While `neklab` uses `LightKrylov`'s abstract interfaces and implementation of the Krylov-Schur algorithm, `KTH Framework` relies under the hood on `ARPACK` and its reversed communication methodology.
...
The numerical settings regarding the `matvec`, in particular those internal to `Nek5000`, are identical in both computations. Due to the algorithmic differences between the Arnoldi iteration in `LightKrylov` and the implementation using IRAM in `ARPACK`, the settings for the Krylov iterations are adapted for each method. The code to run this example can be found [here](?).
From [Figure @fig:timings], it can be seen that the `eigs` solver in `LightKrylov` correctly computes the leading eigenpair of the spectrum (computed with `ARPACK`) and that, as for the fixed-point iteration, the use of extended `abstract` types and object-oriented programming in Fortran incurs a negligible computational overhead for such large-scale applications.
