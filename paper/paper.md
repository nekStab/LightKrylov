...
Using the supercritical two-dimensional flow past a circular cylinder at Reynolds number of 100 as an example, the top row in figure @fig:timings depicts the streamwise (*u*) velocity distribution of the converged flow field (top) as well as the pointwise squared difference between the the solutions obtained with the two solution techniques with a tolerance of $\epsilon = 10^{-8}$, namely `LightKrylov`'s Newton-GMRES solver and the selective frequency damping algorithm [@pof-sfd] (often considered as the gold standard in this community). Note that the error norm is different for each method. The Newton-GMRES iteration uses the residual of the nonlinear integration over a time horizon of $\tau = 1.0$ integrated over the domain as a convergence measure, which is more strict than the point-wise measure used in the implementation of SFD.
