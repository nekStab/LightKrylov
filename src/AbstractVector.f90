module lightkrylov_AbstractVector
  !! This module provides the base class `abstract_real_vector` from which all Krylov vectors need to be derived.
  ...
  !! - `axpby(self, alpha, vec, beta)` : A function computing **in-place** the result of \( \mathbf{x} = \alpha \mathbf{x} + \beta \mathbf{y} \) with \( \mathbf{x} \) and \( \mathbf{y} \) two similar instances derived from `abstract_real_vector` and \( \alpha, \beta \in \mathbb{R} \).
  !! - `dot(self, vec)` : A function computing the inner product between two similar instances derived from `abstract_real_vector`, i.e. \( \alpha = \mathbf{x}^T \mathbf{y} \) (with \( \alpha \in \mathbb{R} \)).
  !!
  ...
  !!     + `y` is an `abstract_real_vector`.
  !!     + `X` is an array of `abstract_real_vector`
  ...
  !!     + `C` is an array of `abstract_real_vector` of size \(q\).
  !!     + `A` is an array of `abstract_real_vector` of size \(r\).
  ...
  !!     + `A` is an array of `abstract_real_vector` of size \(r\).
  !!     + `B` is an array of `abstract_real_vector` of size \(q\).
  ...
  !!     + `A` and `B` are two arrays for `abstract_real_vector` with the same size (and type).
  !!     + `alpha` and `beta` are two double precision floating point numbers.
  !! - `mat_zero(A)` : return \( \mathbf{A} = \mathbf{0} \) where
  !!     + `A` is an array of `abstract_real_vector`.
  !! - `mat_copy(A, B)` : return \( \mathbf{A} = \mathbf{B} \) where
  !!     + `A` and `B` are two arrays of `abstract_real_vector` with the same size.
  !!
  !! @warning
  ...
  !! @endwarning
   ...
   type, abstract, public :: abstract_real_vector
   ...
   contains
      ...
      procedure(abstract_real_zero), deferred, public              :: zero
      !! Sets an `abstract_real_vector` to zero.
      procedure(abstract_real_rand), deferred, public              :: rand
      !! Create a random `abstract_real_vector.
      procedure(abstract_real_scal), deferred, public              :: scal
      !! Compute the scalar-vector product.
      procedure(abstract_real_axpby), deferred, pass(self), public :: axpby
      ...
      !! Computes the dot product between two `abstract_real_vector`.
      procedure, pass(self), public                                :: norm => real_norm
      !! Compute the norm of an `abstract_real_vector`.
   end type abstract_real_vector

   ! Abstract interfaces for the type-bound procedures.
   abstract interface
      subroutine abstract_real_zero(self)
        !! Abstract interface to zero-out a vector in-place.
         import abstract_real_vector
         class(abstract_real_vector), intent(inout) :: self
         !! Vector to be zeroed-out.
      end subroutine abstract_real_zero

      subroutine abstract_real_rand(self, ifnorm)
         !! Abstract interface to set vector to (normalized) random vector
         import abstract_real_vector
         class(abstract_real_vector), intent(inout) :: self
         logical, optional,      intent(in)    :: ifnorm
      end subroutine abstract_real_rand

      subroutine abstract_real_scal(self, alpha)
        !! Abstract interface for in-place scalar-vector multiplication.
         import abstract_real_vector, wp
         class(abstract_real_vector), intent(inout) :: self
         !! Input/Output vector.
         real(kind=wp), intent(in)             :: alpha
         !! Scalar multiplier.
      end subroutine abstract_real_scal
      ...
      real(kind=wp) function abstract_real_dot(self, vec) result(alpha)
        !! Abstract interface to compute the dot product between two `abstract_real_vector`.
         import abstract_real_vector, wp
         class(abstract_real_vector), intent(in) :: self, vec
...
contains
   ...
   subroutine copy(out, from)
     !! This function copies the content in `from` to `out`.
      class(abstract_real_vector), intent(in)               :: from
     !! Vector we wish to copy from.
      class(abstract_real_vector), allocatable, intent(out) :: out
   ...
   subroutine add(self, y)
     !! Add two `abstract_real_vector` in-place.
      class(abstract_real_vector), intent(inout) :: self
     !! Input/Output vector.
      class(abstract_real_vector), intent(in)    :: y
   ...
   subroutine sub(self, y)
     !! Subtract two `abstract_real_vector` in-place.
      class(abstract_real_vector), intent(inout) :: self
     !! Input/Output vector.
      class(abstract_real_vector), intent(in)    :: y
   ...
   real(kind=wp) function real_norm(self) result(alpha)
     !! Compute the norm of an `abstract_real_vector`. Note that it is based
     !! on the subroutine `dot` which needs to be provided by the user.
      class(abstract_real_vector), intent(in) :: self
     !! Vector whose norm is computed.
      alpha = sqrt(self%dot(self))
   end function real_norm
   ...
   subroutine get_vec(y, X, v)
     ...
     !! `y` is an `abstract_real_vector`, `X` an array of `abstract_real_vector` and `v` a Fortran
     !! array containing the coefficients of the linear combination.
      class(abstract_real_vector), allocatable, intent(out) :: y
     !! Output vector.
      class(abstract_real_vector), dimension(:), intent(in) :: X
     !! Krylov basis.
   ...
   subroutine mat_mult_direct(C, A, B)
     ...
     !! `A` and `C` are arrays of `abstract_real_vector` and `B` a real-valued matrix.
      class(abstract_real_vector), intent(out) :: C(:)
     !! Array of `abstract_real_vector` containing the result of the operation.
      class(abstract_real_vector), intent(in)  :: A(:)
     !! Array of `abstract_real_vector` to be right-multiplied by `B`.
   ...
   subroutine mat_mult_transpose(C, A, B)
      ...
      !! `C` is a real-valued matrix while `A` and `B` are arrays of `abstract_real_vector`.
      class(abstract_real_vector), intent(in)  :: A(:)
       !! Array of `abstract_real_vector`.
   ...
   subroutine mat_axpby_avecmat(A, alpha, B, beta)
      !! Utility function to compute in-place the result \( \mathbf{A} = \alpha \mathbf{A} + \beta \mathbf{B} \)
      !! where `A` and `B` are arrays of `abstract_real_vector`.
      class(abstract_real_vector), intent(inout)  :: A(:)
       !! Input/Output array of `abstract_real_vector`.
      class(abstract_real_vector), intent(in)     :: B(:)
       !! Array of `abstract_real_vector` to be added/subtracted to `A`.
      real(kind=wp), intent(in)     :: alpha, beta
       !! Scalar multipliers.
   ...
   subroutine mat_zero(A)
      !! Initialize an array of `abstract_real_vector` to zero by repeatedly calling
      !! `A(i)%zero()` for `i = 1, size(A)`.
      class(abstract_real_vector), intent(inout)  :: A(:)
      !! Array of `abstract_real_vector` to be zeroed-out.
   ...
   subroutine mat_copy(A, B)
      !! Utility function to copy an array of `abstract_real_vector` into another one.
      class(abstract_real_vector), intent(out)  :: A(:)
      !! Output array.
      class(abstract_real_vector), intent(in)   :: B(:)
