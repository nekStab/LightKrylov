...
contains
   ...
   subroutine set_timer_switch(value)
   ...
   end subroutine set_timer_switch
   ...
   integer function get_timer_count(self) result(count)
      !! Type-bound to abstract_watch: Getter routine to return the timer count
      class(abstract_watch) :: self
   ...
   subroutine add_group(self, name, istart, iend)
      !! Type-bound to abstract_watch: Add timer group to watch
      !! Note: The new group name must be unique
      class(abstract_watch), intent(inout) :: self
      character(len=*), intent(in) :: name
      ...
      if (self%group_count == 0) then
         ...
         self%groups(1) = lightkrylov_timer_group(name=to_lower(name), istart=istart, iend=iend)
         self%group_count = 1
      else
         if (self%get_group_id(name) > 0) then
            call stop_error('Timer group "'//to_lower(trim(name))//'" already defined!', & 
                              & module=this_module, procedure='add_group')
         end if
         self%groups = [ self%groups, lightkrylov_timer_group(name=to_lower(name), istart=istart, iend=iend) ]
         self%group_count = self%group_count + 1
      end if
      call logger%log_debug('Timer group "'//to_lower(trim(name))//'" added.', module=this_module)
   ...
   subroutine finalize(self)
      ...
      call logger%log_message(msg, module=this_module)
      write(msg, '(2X,A40,I5)') 'Called timers:', sum(ic) + ic_user
      call logger%log_message(msg, module=this_module)
      do i = 1, self%group_count
         associate(g => self%groups(i))
            call print_summary_header(g%name)
