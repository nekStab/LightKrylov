...
contains
    ...
    module procedure print_pp_gmres_sp
        ! internals
        character(len=*), parameter :: this_procedure = 'print_pp_gmres_sp'
        integer :: i
    ...
    module procedure reset_pp_gmres_sp
        self%n_iter = 0
        self%n_inner = 0
        self%n_outer = 0
    ...
    module procedure print_pp_gmres_dp
        ! internals
        character(len=*), parameter :: this_procedure = 'print_pp_gmres_dp'
        integer :: i
        logical :: ifreset, ifverbose
        character(len=128) :: msg
    ...
    module procedure reset_pp_gmres_dp
        self%n_iter = 0
        self%n_inner = 0
        self%n_outer = 0
        self%converged = .false.
    ...
    module procedure pp_gmres_rsp
       ! Options.
        integer :: kdim, maxiter
        real(sp) :: tol, rtol_, atol_
        logical :: trans
        type(pp_gmres_sp_opts)     :: opts
        type(pp_gmres_sp_metadata) :: gmres_meta
        ...
        ! Miscellaneous.
        character(len=*), parameter :: this_procedure = 'pp_gmres_rsp'
        integer :: i, k, iter
        ...
        if (present(options)) then
            select type (options)
            type is (pp_gmres_sp_opts)
                opts = options
            class default
                call stop_error("The optional intent [IN] argument 'options' must be of type 'pp_gmres_sp_opts'", &
                              & this_module, this_procedure)
            end select
        else
            opts = pp_gmres_sp_opts()
        endif
        ...
        gmres_meta = pp_gmres_sp_metadata() ; gmres_meta%converged = .false.
        ...
        if (present(meta)) then
            select type(meta)
            type is (pp_gmres_sp_metadata)
                meta = gmres_meta
            class default
                call stop_error("The optional intent [OUT] argument 'meta' must be of type 'pp_gmres_sp_metadata'", &
                              & this_module, this_procedure)
            end select
        end if
    ...
    module procedure pp_gmres_rdp
       ! Options.
        integer :: kdim, maxiter
        real(dp) :: tol, rtol_, atol_
        logical :: trans
        type(pp_gmres_dp_opts)     :: opts
        type(pp_gmres_dp_metadata) :: gmres_meta
        ...
        ! Miscellaneous.
        character(len=*), parameter :: this_procedure = 'pp_gmres_rdp'
        integer :: i, k, iter
        ...
        if (present(options)) then
            select type (options)
            type is (pp_gmres_dp_opts)
                opts = options
            class default
                call stop_error("The optional intent [IN] argument 'options' must be of type 'pp_gmres_dp_opts'", &
                              & this_module, this_procedure)
            end select
        else
            opts = pp_gmres_dp_opts()
        endif
        ...
        ! Initialize metadata and & reset matvec counter
        gmres_meta = pp_gmres_dp_metadata() ; gmres_meta%converged = .false.
        call A%reset_counter(trans, 'pp_gmres%init')
        ...
        if (present(meta)) then
            select type(meta)
            type is (pp_gmres_dp_metadata)
                meta = gmres_meta
            class default
                call stop_error("The optional intent [OUT] argument 'meta' must be of type 'pp_gmres_dp_metadata'", &
                              & this_module, this_procedure)
            end select
        end if
    ...
    module procedure pp_gmres_csp
       ! Options.
        ...
        ! Miscellaneous.
        character(len=*), parameter :: this_procedure = 'pp_gmres_csp'
        integer :: i, k, iter
    ...
    module procedure pp_gmres_cdp
       ! Options.
        ...
        ! Miscellaneous.
        character(len=*), parameter :: this_procedure = 'pp_gmres_cdp'
        integer :: i, k, iter
