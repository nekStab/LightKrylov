module LightKrylov_TestUtils
    ...
    contains
       ...
       procedure, pass(self), public :: set_base_state => set_base_sys
    end type roessler
    ...
    contains
       ...
       !procedure, pass(self), public :: get_base_state => get_base_jac
       procedure, pass(self), public :: set_base_state => set_base_jac
    end type jacobian
...
contains
    ...
    subroutine set_base_jac(self, state)
      ...
      type is (state_vector)
         if (.not.allocated(self%X)) then
            allocate(self%X, source=state)
         else
            call self%X%axpby(zero_rdp, state, one_rdp)
         end if
      end select

      return
    end subroutine set_base_jac

    subroutine set_base_sys(self, state)
      class(roessler),            intent(inout) :: self
      class(abstract_vector_rdp), intent(in)    :: state

      call self%jacobian%set_base_state(state)
      
      return
    end subroutine set_base_sys
